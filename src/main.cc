...
#include <lab/types.h>
#include "szs/messages_generated.h"
...
#endif
{
	...
	std::cout << "Ohey we just built an asio thing and it didn't blow up (yet)" << std::endl;

	tcp::endpoint endpoint(tcp::v4(), 16384);
	endpoint.address(asio::ip::address_v4::from_string("127.0.0.1"));
	tcp::acceptor acceptor(io_service, endpoint);
	std::cout << "Acceptor local endpoint: " << acceptor.local_endpoint() << std::endl;
	...
	acceptor.async_accept(socket,
		...
		{
			std::cout << "Err we accepted something?" << std::endl;
			const char *buffer = "hello there\n";
			asio::write(socket, asio::buffer(buffer, strlen(buffer)));

			flatbuffers::FlatBufferBuilder builder(1024);

			auto request = messages::CreateRequest(
				builder,
				builder.CreateString("ping")
			);
